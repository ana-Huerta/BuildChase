...
import HonkaiLightconeModal from './HonkaiLightconeModal'
import { Ban } from 'lucide-react'
import { useSearchParams } from 'react-router-dom'

export default function HonkaiLightconeLayout({ children }) {
          ...
          <main style={{ flex: 1, overflowY: 'auto', padding: '20px' }}>
            <section style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flexDirection: 'row' }}>
                    <div style={{ color: 'var(--text-secondary)', fontFamily: 'MilkyLemon, sans-serif' }}>Vía</div>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    ...
                    <div>
                        <div style={{ position: 'relative', display: 'inline-block' }}>
                            <select value={rarity} onChange={(e) => setParam('rarity', e.target.value)} className="input" style={{ appearance: 'none',WebkitAppearance: 'none', MozAppearance: 'none',  padding: '8px 36px 8px 12px', borderRadius: 8, background: 'rgba(53, 66, 140, .4)', border: '1px solid var(--border)', color: 'var(--text-secondary)', minWidth: 120}}>
