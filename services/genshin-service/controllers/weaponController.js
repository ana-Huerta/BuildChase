...
exports.getAll = async (req, res) => {
	try {
		...
		const items = await Weapon.find(filters).populate('recommendedCharacters');
...
exports.getById = async (req, res) => {
	try {
		const item = await Weapon.findById(req.params.id).populate('recommendedCharacters');
		if (!item) return res.status(404).json({ success: false, message: 'Weapon not found' });
...
exports.create = async (req, res) => {
	try {
		const newItem = new Weapon(req.body);
...
exports.remove = async (req, res) => {
	try {
		const removed = await Weapon.findByIdAndDelete(req.params.id);
		if (!removed) return res.status(404).json({ success: false, message: 'Weapon not found' });
